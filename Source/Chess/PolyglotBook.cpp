...
int RemapCastlingDestination(int from, int to)
{
    ...
    {
        if (to == SquareIndex(7, 0))
            return SquareIndex(6, 0);  // e1h1 -> e1g1 (White O-O)
        if (to == SquareIndex(0, 0))
            return SquareIndex(2, 0);  // e1a1 -> e1c1 (White O-O-O)
    }
    else if (from == SquareIndex(4, 7))
    {
        if (to == SquareIndex(7, 7))
            return SquareIndex(6, 7);  // e8h8 -> e8g8 (Black O-O)
        if (to == SquareIndex(0, 7))
            return SquareIndex(2, 7);  // e8a8 -> e8c8 (Black O-O-O)
    }
    return to;
}

char PromotionLetter(int promotionField)
{
    // Polyglot's 3-bit promotion field: 0 = none, 1 = knight, 2 = bishop, 3 = rook, 4 = queen.
    switch (promotionField)
    {
    case 1:
        return 'n';
    case 2:
...
std::string DecodeMove(std::uint16_t raw)
{
    ...
    const int from = (raw >> 6) & 0x3F;
    const int promotion = (raw >> 12) & 0x7;

    const int remappedTo = RemapCastlingDestination(from, to);

    std::string uci = SquareToAlgebraic(from) + SquareToAlgebraic(remappedTo);
    if (const char promoLetter = PromotionLetter(promotion); promoLetter != '\0')
        uci += promoLetter;

    return uci;
}

int SquareFromAlgebraic(const std::string& uci, std::size_t offset)
{
    return SquareIndex(uci[offset] - 'a', uci[offset + 1] - '1');
}
}  // namespace

std::uint64_t PolyglotBook::ComputeHash(const BoardState& board, PieceColor sideToMove, CastlingRights castling, std::optional<int> enPassantTarget)
{
    std::uint64_t hash = 0;

    for (int square = 0; square < 64; ++square)
    {
        if (const std::optional<Piece>& piece = board[square])
            hash ^= kRandom64[64 * PieceKindIndex(*piece) + square];
    }
    ...
    if (enPassantTarget)
    {
        ...
        {
            for (const int adjFile : {file - 1, file + 1})
            {
                if (adjFile < 0 || adjFile > 7)
                    continue;

                const std::optional<Piece>& adjacent = board[SquareIndex(adjFile, pawnRank)];
                if (adjacent && adjacent->Type == PieceType::Pawn && adjacent->Color == sideToMove)
                    capturable = true;
            }
        }

        if (capturable)
            hash ^= kRandom64[772 + file];
    }
...
bool PolyglotBook::Load(const std::filesystem::path& path)
{
    Clear();
    ...
    if (!file)
    {
        LOG_ERROR("PolyglotBook::Load: couldn't open '{}'", path.string());
        return false;
    }

    const std::streamoff size = file.tellg();
    constexpr std::streamoff kEntrySize = 16;
    if (size <= 0 || size % kEntrySize != 0)
    {
        LOG_ERROR("PolyglotBook::Load: '{}' is not a valid Polyglot book (size {} isn't a positive multiple of {} bytes)", path.string(), static_cast<long long>(size), static_cast<long long>(kEntrySize));
        return false;
    }
    file.seekg(0, std::ios::beg);

    const std::size_t entryCount = static_cast<std::size_t>(size) / static_cast<std::size_t>(kEntrySize);
    m_Entries.reserve(entryCount);

    std::array<unsigned char, kEntrySize> raw{};
    for (std::size_t i = 0; i < entryCount; ++i)
    {
        if (!file.read(reinterpret_cast<char*>(raw.data()), raw.size()))
        {
            LOG_ERROR("PolyglotBook::Load: '{}' truncated while reading entry {}/{}", path.string(), i, entryCount);
            m_Entries.clear();
            return false;
        }

        // All multi-byte fields are big-endian on disk (Polyglot spec) - this host is
        // little-endian (x86/x64), so byte-swap on read.
        std::uint64_t key = 0;
        for (int b = 0; b < 8; ++b)
            key = (key << 8) | raw[b];

        const std::uint16_t move = static_cast<std::uint16_t>((raw[8] << 8) | raw[9]);
        const std::uint16_t weight = static_cast<std::uint16_t>((raw[10] << 8) | raw[11]);
        // Bytes 12-15 are the "learn" field - parsed by nothing, intentionally discarded.
...
bool PolyglotBook::IsLoaded() const
{
    return !m_Entries.empty();
}

std::size_t PolyglotBook::EntryCount() const
{
    return m_Entries.size();
}

std::optional<std::string> PolyglotBook::FindMove(const BoardState& board, PieceColor sideToMove, CastlingRights castling,
                                                    std::optional<int> enPassantTarget, SelectionMode mode) const
{
    if (m_Entries.empty())
        return std::nullopt;

    const std::uint64_t hash = ComputeHash(board, sideToMove, castling, enPassantTarget);
    ...
    const Entry* chosen = best;
    if (mode == SelectionMode::WeightedRandom && totalWeight > 0)
    {
        thread_local std::mt19937 rng{std::random_device{}()};
        std::uint32_t draw = std::uniform_int_distribution<std::uint32_t>(0, totalWeight - 1)(rng);
        for (auto it = begin; it != end; ++it)
        {
            if (it->Weight == 0)
                continue;
    ...
    {
        LOG_WARN("PolyglotBook::FindMove: decoded move '{}' doesn't match the board (no {} piece on the from-square) - ignoring, possible hash collision or mismatched book", uci, sideToMove == PieceColor::White ? "White" : "Black");
        return std::nullopt;
    }
